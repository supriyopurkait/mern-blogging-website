...
const BlogInteraction = () => {
  ...
  useEffect(() => {
    if(access_token) {
      axios.post(import.meta.env.VITE_SERVER_URL + "/isliked-by-user", {
        _id
      }, {
        headers: {
  ...
  const handelLike = () => {
    if(access_token) {
      ...
      axios.post(import.meta.env.VITE_SERVER_URL + "/like-blog", {
        _id,
    ...
    <>
      ...
      <div className="flex gap-6 justify-between">
        <div className="flex gap-3 items-center">
          <button 
            className={`w-10 h-10 rounded-full flex items-center justify-center ${isLikedByUser ? "bg-[#efbebe]" : "bg-grey/80"}`} 
            onClick={handelLike}
          >
            {isLikedByUser ? (
        ...
        <div className="flex gap-6 items-center">
          {username === author_username && (
            <Link to={`/editor/${blog_id}`} className="undeline hover:text-purple">
              Edit
            </Link>
          )}
          <Link to={`https://twitter.com/intend/tweet?${title}&url=${location.href}`}>
